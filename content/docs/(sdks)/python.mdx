...
In order to accomplish this, we persist cookies to disk with `pickle` and reload them on startup.

Note: do **not** pickle the live `CookieJar` object directly (it contains a lock and will fail). Instead, pickle the list of cookies and rebuild a jar on load.
...
with vrchatapi.ApiClient(configuration) as api_client:
    ...
    # If a saved cookie file exists, rebuild a CookieJar and load it.
    ...
    # Save the cookies (not the jar object itself) after successful login/use.
    with open(COOKIE_FILE, "wb") as fh:
        pickle.dump(list(api_client.rest_client.cookie_jar), fh)
```
